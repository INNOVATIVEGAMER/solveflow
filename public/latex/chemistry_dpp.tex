...
\begin{answerbox}{B}
Cu has a higher reduction potential, so it acts as the \textbf{cathode}; Zn is the \textbf{anode}:
\begin{align}
  E^\circ_\text{cell} &= E^\circ_\text{cathode} - E^\circ_\text{anode} \\
                      &= (+0.34) - (-0.76) \\
                      &= \boxed{+1.10\,\text{V}}
\end{align}
Cell reaction: $\text{Zn} + \text{Cu}^{2+} \yields \text{Zn}^{2+} + \text{Cu}$
\end{answerbox}

\begin{notebox}
$\Delta G^\circ = -nFE^\circ_\text{cell}$.\quad For the Daniell cell: $n=2$, so
$\Delta G^\circ = -2 \times 96485 \times 1.10 \approx -212\,\text{kJ/mol}$ (spontaneous).
\end{notebox}
...
\begin{align}
  t_{1/2} &= \frac{\ln 2}{k} = \frac{0.693}{k} \\[6pt]
  k &= \frac{0.693}{t_{1/2}} = \frac{0.693}{693\,\text{s}} = \boxed{1.0 \times 10^{-3}\,\text{s}^{-1}}
\end{align}
\end{answerbox}

\begin{notebox}
First-order: $t_{1/2} = 0.693/k$ — \textbf{independent of initial concentration}.\\
Second-order: $t_{1/2} = 1/(k[A]_0)$ — \textbf{depends on initial concentration}.
\end{notebox}
...
\midrule
\rowcolor{rowA} Force & Van der Waals & Chemical bond \\
\rowcolor{rowB} Reversibility & Reversible & Irreversible \\
\rowcolor{rowA} Specificity & Non-specific & Highly specific \\
\rowcolor{rowB} $\Delta H_{ads}$ & 20--40 kJ/mol & 40--400 kJ/mol \\
\rowcolor{rowA} Activation energy & Low/None & High \\
\rowcolor{rowB} Layers & Multilayer & Monolayer \\
\bottomrule
\end{tabular}
\end{center}

Physisorption is $\boxed{\text{reversible, non-specific, and involves van der Waals forces}}$.
\end{answerbox}

\begin{notebox}
BET adsorption isotherm applies to physisorption (multilayers).
Langmuir adsorption isotherm assumes monolayer (applies to chemisorption).
\end{notebox}
...
\begin{answerbox}{A}
All three molecules are pyramidal ($sp^3$ hybridised central atom, one lone pair):
...
\toprule
\rowcolor{accentcolor}\color{white}Molecule & \color{white}Bond Angle & \color{white}Reason \\
\midrule
\rowcolor{rowB} NH$_3$ & $107.8^\circ$ & H is less electronegative; bond pairs closer to N \\
\rowcolor{rowA} NF$_3$ & $102.5^\circ$ & F withdraws e$^-$ density; bp--bp repulsion reduced \\
\rowcolor{rowB} PH$_3$ & $93.3^\circ$  & P is larger; poor $p$-orbital overlap; lp more diffuse \\
\bottomrule
\end{tabular}
\end{center}

$\therefore\; \boxed{\text{NH}_3\,(107.8^\circ) > \text{NF}_3\,(102.5^\circ) > \text{PH}_3\,(93.3^\circ)}$
\end{answerbox}

\begin{notebox}
In PH$_3$: P uses nearly pure $p$-orbitals for bonding (lone pair is in $s$-orbital),
so the bond angle is close to $90^\circ$.
\end{notebox}
...
\begin{align}
  ...
  x &= -1 + 8 = \boxed{+7}
\end{align}
Formula: $\text{MnO}_4^-$ (permanganate ion).
Mn$_2$O$_7$ is the anhydride (Mn = +7 also) but it is a covalent compound, not the ion.
\end{answerbox}

\begin{notebox}
Common Mn oxidation states: $+2$ (MnO, pale pink), $+4$ (MnO$_2$, brown),
$+6$ (MnO$_4^{2-}$, manganate, green), $+7$ (MnO$_4^-$, permanganate, purple).
\end{notebox}
...
\begin{answerbox}{B}
\textbf{Step 1 — Identify ligands and metal:}
\begin{itemize}[noitemsep]
  \item 4 × NH$_3$ = tetraammine
  \item Cl$^-$ = chlorido (alphabetically before nitrito)
  \item NO$_2^-$ via O = nitrito-$\kappa O$
  \item Metal: Co; charge: $+3$ (from overall neutral complex with one outer Cl$^-$)
\end{itemize}

\textbf{Step 2 — Alphabetical order of ligands:} chlorido $<$ nitrito $<$ tetraammine

\textbf{Name:} $\boxed{\text{Tetraamminechloridonitrito-}\kappa O\text{-cobalt(III) chloride}}$
\end{answerbox}

\begin{notebox}
Ligands named alphabetically ignoring multiplying prefixes (di, tri, tetra).
$\kappa O$ indicates the nitrito ligand is $O$-bonded (vs $N$-bonded = nitro).
\end{notebox}
...
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{>{\bfseries}p{3.5cm}>{\centering\arraybackslash}p{2.5cm}>{\centering\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{3cm}}
\toprule
\rowcolor{accentcolor}\color{white}Compound & \color{white}Tollens' & \color{white}Fehling's & \color{white}Reason \\
\midrule
\rowcolor{rowB} Formaldehyde & +ve & +ve & Aliphatic aldehyde \\
\rowcolor{rowA} Acetaldehyde & +ve & +ve & Aliphatic aldehyde \\
\rowcolor{rowB} \textbf{Benzaldehyde} & \textbf{+ve} & \textbf{$-$ve} & Aromatic aldehyde \\
\rowcolor{rowA} Acetone & $-$ve & $-$ve & Ketone \\
\bottomrule
\end{tabular}
\end{center}

Benzaldehyde is an aldehyde, so it reduces Tollens' reagent (Ag$^+$ $\to$ Ag$\downarrow$, silver mirror).
But aromatic aldehydes \emph{cannot} reduce Cu$^{2+}$ in Fehling's reagent — only aliphatic
aldehydes do. $\boxed{\text{Answer: (C) Benzaldehyde}}$
\end{answerbox}

\begin{notebox}
Tollens' test: \emph{all} aldehydes react. Fehling's test: only \emph{aliphatic} aldehydes react.
Ketones fail both (no $-$CHO group).
\end{notebox}
...
\begin{answerbox}{B}
In aqueous solution, two effects compete:
\begin{enumerate}[noitemsep]
  \item \textbf{Inductive (+I) effect} — more methyl groups $\to$ higher electron density on N $\to$ stronger base.
  \item \textbf{Solvation effect} — more methyl groups $\to$ more steric hindrance $\to$ less solvation of the cation $\to$ weaker base.
\end{enumerate}
The secondary amine (CH$_3$)$_2$NH achieves the best balance:
\[
  \boxed{(CH_3)_2NH\;>\; CH_3NH_2\;>\; (CH_3)_3N\;>\; NH_3}
\]
\end{answerbox}

\begin{notebox}
In \emph{gas phase}, order follows +I effect: (CH$_3$)$_3$N $>$ (CH$_3$)$_2$NH $>$ CH$_3$NH$_2$ $>$ NH$_3$.
In \emph{aqueous solution}, solvation inverts the order of secondary vs. tertiary.
\end{notebox}
...
\begin{answerbox}{B}
\[
  n\;\underbrace{H_2N-(CH_2)_6-NH_2}_{\text{Hexamethylenediamine}}
  \;+\; n\;\underbrace{HOOC-(CH_2)_4-COOH}_{\text{Adipic acid}}
  \;\yields\; [\underbrace{-NH-(CH_2)_6-NH-CO-(CH_2)_4-CO-}_{\text{Nylon-6,6 repeat unit}}]_n + 2nH_2O
\]

Both monomers have \textbf{6 carbon atoms} — hence the name Nylon-\textbf{6,6}.
$\boxed{\text{Answer: (B)}}$

\smallskip
Compare: \textbf{Nylon-6} is made from caprolactam (ring-opening of a single 6-C monomer).
\end{answerbox}

\begin{notebox}
Nylon-6,6: two different monomers (diamine + diacid), condensation polymer.\\
Nylon-6: one monomer (caprolactam), addition polymer via ring opening.
\end{notebox}
...
\begin{tikzpicture}[scale=0.8, font=\small]
  % Helix backbone schematic
  \foreach \i in {0,1,2,3,4}{
    \draw[thick,accentcolor] (0,\i*0.7) arc (180:0:1.2 and 0.35);
    \draw[thick,accentcolor,dashed] (2.4,\i*0.7+0.35) arc (0:-180:1.2 and 0.35);
  }
  % H-bond arrows (endpoints computed explicitly to avoid arithmetic in coords)
  \draw[->,dashed,red!70!black,thick] (2.4,0.35)--(0,0.88) node[midway,right,font=\tiny]{H-bond};
  \draw[->,dashed,red!70!black,thick] (2.4,1.05)--(0,1.58) node[midway,right,font=\tiny]{H-bond};
  \draw[->,dashed,red!70!black,thick] (2.4,1.75)--(0,2.28) node[midway,right,font=\tiny]{H-bond};
  \node[below] at (1.2,-0.3) {$\alpha$-helix (schematic)};
  \node[right] at (2.6,1.8) {\tiny C$=$O$\cdots$H$-$N};
\end{tikzpicture}
\end{center}

The \textbf{secondary structure} ($\alpha$-helix, $\beta$-sheet) is stabilised by
\textbf{hydrogen bonds} between the $\text{C}=\text{O}$ of one peptide bond and the
$\text{N}-\text{H}$ of another (4 residues away in an $\alpha$-helix).

$\boxed{\text{Answer: (C)}}$
\end{answerbox}

\begin{notebox}
Structure hierarchy: Primary = peptide bonds; Secondary = H-bonds;
Tertiary = H-bonds, disulfide bridges, hydrophobic, ionic;
